...
err_t sys_exec(u64 path, char **argv, u64 envp) {
	...
	// 从旧的用户栈拷贝参数到新的用户栈
	log(DEBUG, "START COPY ARGV\n");
	td_initustack(td, TD_TEMPUSTACK);
	td_setustack(td, argc_count(td->td_pt, argv), argv);
	log(DEBUG, "END COPY ARGV\n");
	// 将旧的用户栈回收，新的用户栈生效
	log(DEBUG, "START SWITCH USTACK\n");
	...
	log(DEBUG, "END SWITCH USTACK\n");

	// 回收先前的代码段
	log(DEBUG, "START RECLAIM CODE\n");

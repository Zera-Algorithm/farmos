 ...
 */
static int dirLookup(FileSystem *fs, Dirent *dir, char *name, struct Dirent **file, longEntSet *longSet) {
	u32 i, j;
    FAT32Directory *f;
...
static int walkPath(FileSystem *fs, char *path, Dirent *baseDir, Dirent **pdir, Dirent **pfile, char *lastelem, longEntSet *longSet) {
    printf("walk pathing...\n");
    char *p;
 ...
 * @param path 文件或目录的路径。如果path是绝对路径，则忽略baseDir；如果path。path指向的地址要求为内核地址
 * @return NULL表示失败
 */
...
int createFile(struct Dirent *baseDir, char *path, Dirent **file) {
    ...
    TRY(dirAllocFile(dir, &f, lastElem));
...
int fileRead(struct Dirent *file, int user, u64 dst, uint off, uint n) {
    ...
    for (; end >= clusIndex * clusSize; clusIndex++) {
        clusterRead(file->fileSystem, clus, 0, (void *)(dst + len), MIN(clusSize, n - len), user);
        clus = fatRead(file->fileSystem, clus);
...
int fileWrite(struct Dirent *file, int user, u64 src, uint off, uint n) {
    ...
    clus = fatRead(file->fileSystem, clus);
    for (; end >= clusIndex * clusSize; clusIndex++) {
        clusterWrite(file->fileSystem, clus, 0, (void *)(src + len), MIN(clusSize, n - len), user);
        clus = fatRead(file->fileSystem, clus);
        len += MIN(clusSize, n - len);
    }
...
int linkAt(struct Dirent *oldDir, char *oldPath, struct Dirent *newDir, char *newPath) {
    ...
    TRY(createFile(newDir, newPath, &newFile));

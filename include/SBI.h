...
#define SBI_SET_TIMER(stime_value) \
    SBI_ECALL(SBI_TIMER_EID, SBI_SET_TIMER_FID, \
    stime_value, 0, 0, 0, 0)

#define SBI_SEND_IPI(hart_mask, hart_mask_base) \
    SBI_ECALL(SBI_IPI_EID, SBI_IPI_SEND_IPI_FID, \
    hart_mask, hart_mask_base, 0, 0, 0)

#define SBI_RFENCE_FENCEI(hart_mask, hart_mask_base) \
    SBI_ECALL(SBI_RFENCE_EID, SBI_RFENCE_FENCEI_FID, \
    hart_mask, hart_mask_base, 0, 0, 0)

#define SBI_RFENCE_SFENCE_VMA(hart_mask, hart_mask_base, start_addr, size) \
    SBI_ECALL(SBI_RFENCE_EID, SBI_RFENCE_SFENCE_VMA_FID, \
...
#define SBI_HART_START(hartid, start_addr, opaque) \
    SBI_ECALL(SBI_HSM_EID, SBI_HSM_HART_START_FID, \
...
#define SBI_HART_STOP() \
    SBI_ECALL(SBI_HSM_EID, SBI_HSM_HART_STOP_FID, \
    0,0,0,0,0)

// 获取hartid的状态，错误存储在error, 返回值存储在value
#define SBI_HART_GET_STATUS(hartid) \
    SBI_ECALL(SBI_HSM_EID, SBI_HART_GET_STATUS_FID, \
    hartid, 0, 0, 0, 0)

#define SBI_HART_SUSPEND(suspend_type, resume_addr, opaque) \
    SBI_ECALL(SBI_HSM_EID, SBI_HSM_HART_SUSPEND_FID, \
    suspend_type, resume_addr, opaque, 0, 0)

#define SBI_SYSTEM_RESET(reset_type, reset_reason) \
    SBI_ECALL(SBI_SYSTEM_RESET_EID, SBI_SYSTEM_RESET_FID, \
    reset_type, reset_reason, 0, 0, 0)
